...
window.align = async function () {
  let zoom = await new Promise((resolve) => {
    remote.getCurrentWindow().webContents.executeJavaScript("document.querySelector('webview').getBoundingClientRect().width", (result) => {
      resolve(result)
    })
  })
  zoom = zoom / 800
...
const handleDOMContentLoaded = () => {
  ...
  const flashQuality = 'height'
  const flashWindowMode = 'window'
  let count = -1
  const t = setInterval(() => {
    try {
      count++
